 ...
 * Assignment 7
 * Name: Keisha Arnold
 * Description: Graph BFS and DFS implementation.
 */
...
{
    // FIXME: Implement
	// initialize set of reachable vertices and add v(i) to a stack
	struct Deque *reachableStk = dequeNew();
	clearVisited(graph);
	dequePushBack(reachableStk, source);
	
	// while stack is not empty
	while(!dequeIsEmpty(reachableStk)) {
		// get and remove (pop) last vertex v from stack
		Vertex *v = dequeFront(reachableStk);
		dequePopFront(reachableStk);
		
		// if v == destination, return true
		if(v == destination) {
			dequeDelete(reachableStk);
			return 1;
		}
		
		// For all neighbors, v(j) of v, if v(j) is not in reachable, add to stack
		for(int i = 0; i < v -> numNeighbors; i++) {
			if(v -> neighbors[i] -> isVisited == 0) {
				dequePushBack(reachableStk, v -> neighbors[i]);
				v -> neighbors[i] -> isVisited = 1;
			}
		}
		v -> isVisited = 1;
	}
...
int bfsIterative(Graph* graph, Vertex* source, Vertex* destination)
{
    // FIXME: Implement
	// initialize set of reachable vertices and add v(i) to a stack
	struct Deque *reachableQ = dequeNew();
	clearVisited(graph);
	dequePushBack(reachableQ, source);
	
	// while stack is not empty
	while(!dequeIsEmpty(reachableQ)) {
		// get and remove (pop) last vertex v from stack
		Vertex *v = dequeFront(reachableQ);
		dequePopFront(reachableQ);
		
		// if v == destination, return true
		if(v == destination) {
			dequeDelete(reachableQ);
			return 1;
		}
		...
		for(int i = 0; i < v -> numNeighbors; i++) {
			if(v -> neighbors[i] -> isVisited == 0) {
				dequePushBack(reachableQ, v -> neighbors[i]);
				v -> neighbors[i] -> isVisited = 1;
			}
		}
		v -> isVisited = 1;
